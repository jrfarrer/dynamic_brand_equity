...
fn_load_weekly_file <- function(filename) {
    ...
    read_fwf(paste0(volume_dir, str_replace(filename, ".", "")), 
             fwf_widths(column_widths, column_names), skip = 1, progress = FALSE) %>%
...
We load each of the store datasets and perform data integrity checks. We show that the number of records load is one less than we found with the `wc -l` shell command (due to the header) and that the number of records unique on store / week / upc (`SY`, `GE`, `VEND`, `ITEM`) is the total number of records loaded. 
...
lapply(tissue_files_list, fn_integrity_check) %>%
    ...
    mutate(
        `Records Loaded` = prettyNum(records_raw, big.mark = ',')
        , `Unique on Store/Week/UPC` = prettyNum(unique_records, big.mark = ',')
    ) %>%
    select(File = file, Records = records_clean, `Records Loaded`, `Unique on Store/Week/UPC`, `Pass?`) %>%
...
Now, let's check that this dataset is indeed at the store (iri_key) - week - upc level of detail. 
